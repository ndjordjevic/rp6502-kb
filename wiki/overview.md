...
The wiki now contains a layer of 6502 **programming knowledge** drawn from Leventhal's *6502 Assembly Language Programming, 2nd Ed.* (1986), Leventhal's *6502 Assembly Language Subroutines* (1982), and Roger Wagner's *Assembly Lines: The Complete Book* (2014). Together these three books form the RP6502 6502 programmer's library — Leventhal for depth, Wagner for accessibility.
...
- **Application snippets** ([[6502-application-snippets]]): string length, blank-skip, parity, hex↔ASCII, BCD-to-7-segment, pattern match. **New (Pass 3)**: Ch.4 code-conversion routines — BN2BCD/BCD2BN (binary↔packed BCD), BN2HEX/HEX2BN (binary↔hex ASCII), BN2DEC (~7,000 cycles, 16-bit signed to decimal ASCII string), DEC2BN (~670 cycles, decimal ASCII string to 16-bit signed); Ch.8 string manipulation — STRCMP (Carry/Zero flag protocol, length-prefixed strings), CONCAT (bounded with overflow truncation), POS substring search (1-based index, abbreviation matching).
- **Interrupt system** ([[6502-interrupt-patterns]]): IRQ/NMI/BRK/RESET vectors, canonical 5-instruction register save/restore sequence, IRQ-vs-BRK distinguish via stack B-bit, polling dispatch, RTI semantics, ISR design rules. **New (Pass 2)**: complete 6522 VIA interrupt-driven I/O pattern (PINTIO — 6 subroutines, single-character buffers, OIE unserviced-interrupt flag); ring-buffer pattern for high-throughput serial I/O; real-time clock/calendar ISR with tick-down chain, leap-year detection, 24-hour wrap.
- **Arithmetic idioms** ([[6502-programming-idioms]]): multi-precision binary/BCD addition, 8-bit multiply (shift-and-add), 8-bit divide (shift-and-subtract), carry-chain rules, 14 6502 quirks quick reference. **New (Leventhal Pass 2)**: 16-bit add (ADD16, 80 cycles), 16-bit sub (SUB16, 80 cycles), 16-bit multiply (MUL16, 650–1100 cycles, 32-bit intermediate product), 16-bit signed/unsigned divide with 4 entry points (SDIV16/UDIV16/SREM16/UREM16, ~1000–1160 cycles), 16-bit comparison with overflow correction (CMP16, ~90 cycles). Bit manipulation (Ch. 7): BITSET/BITCLR/BITTST (57/57/50 cycles) operating on 16-bit words via mask tables; Bit Field Extraction (BFE) and Insertion (BFI); multi-precision logical/arithmetic/rotate shifts. **New (Wagner)**: shift operators (ASL/LSR/ROL/ROR) usage; logical operators (AND/ORA/EOR/BIT) with 65C02 extended BIT modes; BCD mode fundamentals (SED/CLD, decimal add/sub, multi-byte BCD, printing BCD digits, 65C02 BCD flag improvements).
- **Data structures** ([[6502-data-structures]]): unordered/ordered list operations, circular queue, bubble sort, jump tables (pre- and 65C02-style). **New (Pass 3)**: Ch.5 memory operations — MFILL (~11 cycles/byte + 93 overhead, page-at-a-time algorithm), BLKMOV (157 bytes, overlap-safe direction detection); Ch.5 multi-dim array indexing — D1BYTE/D1WORD (73/78 cycles), D2BYTE (~1500 cycles using MUL16), D2WORD, NDIM (Horner evaluation loop for N-dim); RP6502 framebuffer note (D2BYTE maps directly to 320-wide VRAM). Ch.9 operations — ASUM8 (~16cy/byte), ASUM16 (~43cy/word, 24-bit result), BINSCH binary search (O(log₂N) iterations, ~52cy each), BUBSRT (O(N²) stable sort), RAMTST ($55/$AA non-destructive RAM test).
...
- **Sources**: [[picocomputer-intro]] · [[hardware]] · [[rp6502-ria-docs]] · [[rp6502-ria-w-docs]] · [[rp6502-vga-docs]] · [[rp6502-os-docs]] · [[rp6502-github-repo]] · [[release-notes]] · [[quadros-rp2040]] · [[fairhead-pico-c]] · [[pico-c-sdk]] · [[rp2350-datasheet]] · [[w65c02s-datasheet]] · [[leventhal-6502-assembly]] · [[leventhal-subroutines]] · [[wagner-assembly-lines]] · [[youtube-playlist]]
