...
- **Instruction set enhancements** ([[65c02-instruction-set]]): all 65C02 additions over NMOS 6502 — `(zp)` indirect, `JMP (a,x)`, BRA, PHX/PHY/PLX/PLY, STZ, INC A/DEC A, TRB/TSB, SMB/RMB/BBR/BBS; decimal-flag-cleared-on-interrupt fix; JMP indirect page-boundary fix. Plus NMOS quirks (ZP wrap-around, ASL/LSR/ROL/ROR abs,X cycle correction, JMP indirect bug, undocumented NOP skip technique).
- **Interrupt system** ([[6502-interrupt-patterns]]): IRQ/NMI/BRK/RESET vectors, canonical 5-instruction register save/restore sequence, IRQ-vs-BRK distinguish via stack B-bit, polling dispatch, RTI semantics, ISR design rules; complete 6522 VIA interrupt-driven I/O pattern (PINTIO — 6 subroutines, single-character buffers, OIE unserviced-interrupt flag); ring-buffer pattern for high-throughput serial I/O; real-time clock/calendar ISR with tick-down chain, leap-year detection, 24-hour wrap. **Extended (6502.org/Pickens)**: WAI with I=1 inline interrupt (1-cycle latency, no push/pop/vector), ghost interrupt trap, open-drain timing, Pickens 256-byte ring buffer, waveform generator ISR, cross-CPU performance table.
- **Compare instructions** ([[6502-compare-instructions]]): CMP/CPX/CPY complete flag mechanics, branch-after-CMP table, multi-byte equality/unsigned/signed comparisons, N XOR V signed method (BVC+EOR #$80 trick), SO pin alternative. *(from 6502.org, Bruce Clark)*
- **Decimal mode** ([[6502-decimal-mode]]): BCD D-flag, ADC/SBC BCD mechanics, flag validity by CPU variant (6502 only C valid; 65C02 adds N/Z at +1 cycle; V never valid on any; 65C02/65816 auto-clear D on interrupt), undocumented BCD behaviours. *(from 6502.org, Bruce Clark)*
- **Overflow (V) flag** ([[6502-overflow-flag]]): V flag meaning (signed overflow), 6 instructions that affect it, BIT immediate exception (65C02 only — no N/V update), SO pin usage, signed comparison pattern after CMP, decimal mode V. *(from 6502.org, Bruce Clark)*
